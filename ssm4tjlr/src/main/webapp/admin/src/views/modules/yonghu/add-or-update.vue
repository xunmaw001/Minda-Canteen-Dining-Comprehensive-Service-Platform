<template>
  <div class="addEdit-block">
    ...
    >
      ...
      <el-col :span="12">
        <el-form-item class="select" v-if="type!='info'"  label="身份类型" prop="shenfenleixing">
          <el-select v-model="ruleForm.shenfenleixing" placeholder="请选择身份类型">
            <el-option
                v-for="(item,index) in shenfenleixingOptions"
                v-bind:key="index"
                :label="item"
                :value="item">
            </el-option>
        ...
        <div v-else>
          <el-form-item class="input" label="身份类型" prop="shenfenleixing">
	      <el-input v-model="ruleForm.shenfenleixing"
                placeholder="身份类型" readonly></el-input>
          </el-form-item>
        </div>
      </el-col>
      <el-col :span="24">  
      ...
      this.$http({
        ...
        if (data && data.code === 0) {
          ...
          this.$message.error(data.msg);
            ...
            this.shenfenleixingOptions = "学生,老师,游客".split(',')
    },
    // 多级联动参数
    info(id) {
      this.$http({
