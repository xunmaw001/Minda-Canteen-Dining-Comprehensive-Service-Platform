...
<script>
	export default {
		...
		methods: {
			// 支付
			onPayTap(){
				uni.setStorageSync('paytable','discussshangjiacanpin');
			...
			onAcrossTap(tableName){
     			uni.setStorageSync('crossTable','discussshangjiacanpin');
				uni.setStorageSync(`crossObj`, this.detail);
				this.$utils.jump(`../${tableName}/add-or-update?cross=true`);
			},
			// 获取详情
			async init(){
				let res = await this.$api.info('discussshangjiacanpin', this.id);
			...
			async upCallback(mescroll) {
				                let res = await this.$api.list('discussdiscussshangjiacanpin', {
					page: mescroll.num,
					limit: mescroll.size,
					refid: this.id
				});
			...
			// 添加评论
			onCommentTap() {
				this.$utils.jump(`../discussdiscussshangjiacanpin/add-or-update?refid=${this.id}`)
			},
			// 点赞
			async zan() {
				let _this = this;
				uni.showModal({
					...
					success: async function(res) {
						if (res.confirm) {
							...
							await _this.$api.update('discussshangjiacanpin', _this.detail);
			...
			async onFinishTap() {
				...
				await this.$api.update('discussshangjiacanpin', this.detail);
				this.$utils.msg('审核成功');
