		...
		<view class="container">
			...
			<view class="bottom-content cu-bar bg-white tabbar border shop">
				...
				<view style="text-align: right;">
																				                    										<button v-if="isAuth('qiandaoxinxi','审核')" @tap="onSHTap" style="margin-right: 10upx;" class="cu-btn bg-red round shadow-blur" :style="{width:'auto',borderRadius:'8rpx',height:'80rpx',fontSize:'28rpx',color:'#fff',backgroundColor:btnColor[4],borderColor:btnColor[4]}">审核</button>
										                </view>
			</view>
		</view>
...
<script>
	export default {
		...
		methods: {
			// 支付
			onPayTap(){
				uni.setStorageSync('paytable','qiandaoxinxi');
			...
			onAcrossTap(tableName){
     			uni.setStorageSync('crossTable','qiandaoxinxi');
				uni.setStorageSync(`crossObj`, this.detail);
				this.$utils.jump(`../${tableName}/add-or-update?cross=true`);
			},
			// 获取详情
			async init(){
				let res = await this.$api.info('qiandaoxinxi', this.id);
			...
			// 添加评论
			onCommentTap() {
				this.$utils.jump(`../discussqiandaoxinxi/add-or-update?refid=${this.id}`)
			},
			// 点赞
			async zan() {
				let _this = this;
				uni.showModal({
					...
					success: async function(res) {
						if (res.confirm) {
							...
							await _this.$api.update('qiandaoxinxi', _this.detail);
			...
			async onFinishTap() {
				...
				await this.$api.update('qiandaoxinxi', this.detail);
				this.$utils.msg('审核成功');
